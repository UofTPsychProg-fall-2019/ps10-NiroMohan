...
Q3 <- ggplot(ipip, aes(x=gender, y=logMedInc, color=exer, fill=exer, weight=logMedInc)) +
...
ipip.g <- ipip.l %>% group_by(trait, gender, BMI_cat)%>% summarise(traitmean=mean(value))
...
Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=traitmean, group=trait, colour=trait)) + 
